...
function showItems(){
    ...
    displayedArray.forEach((element)=>{
        const item= document.createElement('li');
        item.innerHTML=`Name :${element.Name}<br> Roll No.:${element.RollNo}`;
...
function addGotoFirst(){
    ...
    gotoFirst.innerHTML='<<';
    if(currentPageNo==1){
        gotoFirst.classList.add('inactive');
    }else{
        gotoFirst.classList.remove('inactive');
    }
...
function addGotoLast(){
    ...
    if(currentPageNo==pageCnt){
        gotoLast.classList.add('inactive');
    }else{
        gotoLast.classList.remove('inactive');
    }
